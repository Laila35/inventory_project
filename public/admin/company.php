...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_company'])) {
    ...
    $stmt = $mysqli->prepare("INSERT INTO companies (name, short_name, status, created_at) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $name, $short_name, $status, $created_at);
    $stmt->execute();
    $stmt->close();
...
// Toggle Enable/Disable Company
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['toggle_company_id'])) {
    $id = (int)$_POST['toggle_company_id'];

    $stmt = $mysqli->prepare("SELECT status FROM companies WHERE id=?");
    ...
    $stmt = $mysqli->prepare("UPDATE companies SET status=?, updated_at=NOW() WHERE id=?");
...
// Fetch companies
$companies = $mysqli->query("SELECT * FROM companies ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="sidebar">
    ...
    <a href="company.php" class="active"><i class="bi bi-building"></i> Company</a>
...
<div class="main-content">
    ...
    <div class="breadcrumb-custom">
        <a href="dashboard.php">Dashboard</a> / Company Management
    </div>

    <div class="card mt-3 table-container">
        <div class="card-header">
            <i class="bi bi-building"></i> Company Management
            <button class="btn btn-success btn-sm float-end" data-bs-toggle="modal" data-bs-target="#addCompanyModal">Add</button>
        </div>
        <div class="card-body">
                ...
                <?php if ($companies && $companies->num_rows > 0): ?>
                    <?php while ($row = $companies->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <form action="company.php" method="post" style="display:inline-block;">
                                    <input type="hidden" name="toggle_company_id" value="<?= (int)$row['id'] ?>">
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="6" class="text-center">No companies available</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
...
$(document).ready(function () {
    $('#companyTable').DataTable({ pageLength: 5 });
});
</script>
</body>
</html>
