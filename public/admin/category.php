...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_category'])) {
    ...
    $stmt = $mysqli->prepare("INSERT INTO categories (name, created_at, status) VALUES (?, ?, ?)");
...
// Handle Toggle Enable/Disable Category
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['toggle_category_id'])) {
    $id = $_POST['toggle_category_id'];

    $stmt = $mysqli->prepare("SELECT status FROM categories WHERE id=?");
    ...
    $stmt = $mysqli->prepare("UPDATE categories SET status=? WHERE id=?");
...
<div class="sidebar">
    ...
    <a href="category.php" class="active"><i class="bi bi-tags-fill"></i> Category</a>
...
<div class="content">
    ...
    <div class="breadcrumb-custom">
        <a href="dashboard.php">Dashboard</a> / Category Management
    </div>

    <div class="card mt-3 table-container">
        <div class="card-header">
            <i class="bi bi-tags-fill"></i> Category Management
            <button class="btn btn-success btn-sm float-end" data-bs-toggle="modal" data-bs-target="#addCategoryModal">Add</button>
        </div>
        <div class="card-body">
        ...
        <tr>
            ...
            <td>
                ...
                <form action="category.php" method="post" style="display:inline-block;">
                    <input type="hidden" name="toggle_category_id" value="<?= $row['id']; ?>">
...
$(document).ready(function () {
    $('#categoryTable').DataTable({ pageLength: 5 });
});
</script>
</body>
</html>
